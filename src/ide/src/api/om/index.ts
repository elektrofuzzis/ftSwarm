...
export class RootObjectModel {
  ...
  useBoundStore<T>(
    key: RegistryKey,
    source: () => SequencedDatum<T>,
    mutator: (val: T) => Promise<Sequence>,
    options?: OptimisticStoreOptions & { needsSave?: boolean },
  ): OptimisticStore<T> {
    ...
    createEffect(() => {
      ...
      if (
        updatedSourceValue.seq &&
        (updatedSourceValue.seq >= lastSeenSeq() ||
          updatedSourceValue.seq < lastSeenSeq() - 10)
      ) {
        if (updatedSourceValue.seq < lastSeenSeq() - 10) {
          logger.debug(
            `Sequence overflow detected or forced update. lastSeenSeq: ${lastSeenSeq()}, updatedSourceValue.seq: ${updatedSourceValue.seq}`,
          );
        }
